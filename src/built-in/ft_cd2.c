...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/19 19:21:13 by jomunoz           #+#    #+#             */
/*   Updated: 2025/11/19 19:29:09 by jomunoz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_cd_errors(char *path, int error_id)
{
	...
	message = "bash: cd: error retrieving current directory: getcwd: ";
	if (error_id == 0)
	{
...
int	handle_cd_options(t_cmd *cmd, char *current_pwd)
{
	...
	i = 1;
	if (!ft_strncmp(cmd->args[1], "-P", 3) || !ft_strncmp(cmd->args[1], "-L",
			3))
	{
		write(2,
	...
	{
		while (cmd->args[1][i] && (cmd->args[1][i] == 'P'
				|| cmd->args[1][i] == 'L'))
			i++;
		write(2, "bash: cd: -", 11);
